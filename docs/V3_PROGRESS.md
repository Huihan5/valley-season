...
| 4 | 事件改造与新增 | 已完成 |
| 5 | 调查系统与结局 | 已完成（五个结局均可达；埃莱娜两条碎片待信任来源） |
...
**一 · 行动性信任来源。格雷格已打通，埃莱娜仍然断着。**

交谈性信任上限 +2，所以每个人能不能到 4 全看行动性途径实现了几条。

| NPC | 行动性信任现状 | 实际上限 | 后果 |
|---|---|---|---|
| 格雷格 | 修缮马厩屋顶 +1、**去马厩搭把手 +1**（缺「展现对土地的尊重」） | **4** | 三条碎片与位置线全部打通，4B 可达 |
| 埃莱娜 | **无** | **2** | 两条碎片全部拿不到 |
| 玛莎 | Day 10 请愿 +1、Day 21 营地 +1 | 4 | 可达，但第二条碎片实际要求参加狩猎季 |
| 洛伦茨 | 首次守夜 +1、Day 21 那棵树 +1 | 4 | 可达，同样绑定狩猎季 |

格雷格这条是 2 + 1 + 1 = 4，**零余量**：玩家必须交谈满 6 次、修屋顶、再去搭把手，一样不能少。第三条「展现对土地的尊重」若要补，作者已定绑采伐额度（本季 25 单位不超额 +1），顺带把 GDD 5.4 规定但一直没接的超额声望 -3 一起做掉。那一条到季末才结算，对碎片没有帮助，只是收尾。

埃莱娜是全庄园唯一没有任何行动性信任来源的人。GDD 5.5 给了她两条（保持尊重但不过分关注 +1、保护她不受牵连 +1），**两条都必须实现**——2 + 1 + 1 = 4，只补一条只到 3，`clue_mot_elena_burned`（炉堂烧纸）仍然拿不到。

作者已定「保护她不受牵连」挂 Day 12 税务审计，新增一个「谁经手过这些收条」的判断点。Day 13 的回音（她把办公室重新整理了一遍，桌角多了一支削好的铅笔）现在是无条件播的，接上之后自动成为这一条的回报，一个字都不用改。另一条「保持尊重但不过分关注」尚无场合。
...
| # | 位置 | 缺什么 | 阻塞什么 |
|---|---|---|---|
| 1 | Day 12 税务审计 | 「谁经手过这些收条」判断点的一问一答 | 埃莱娜 +1（她两条碎片的一半） |
| 2 | 埃莱娜第二条途径 | 「保持尊重但不过分关注」的场合与正文 | 她的信任封顶 3，`clue_mot_elena_burned` 拿不到 |
| 3 | 采伐额度 | 不超额时格雷格 +1 的结算文本 | 不阻塞（格雷格已能到 4），只是余量 |
| 4 | `action_results.json` 的 `stable_help` | 「去马厩搭把手」的结果文本，槽已留空 | 不阻塞。行动可用，只是做完没有叙述反馈 |
| 5 | 草稿 4.11 维特第二档 | 那段保守写法的重述正文 | 不阻塞。前后框架已接好，中间空一段 |
| 6 | 结局 4A/4B | 圣火节镜像段落（`festival_mirror` 空槽） | 不阻塞 |
| 7 | 结局 4A | `tookHenkDeal` 的追加段（草稿只写了 4B 与结局三的版本） | 不阻塞 |
| 8 | 炉堂版洛伦茨 | 猎场版先触发时的「已经说过」回应句 | 不阻塞。现在退回普通招呼语 |
| 9 | 夜间审查账目 | 第四晚起的收尾文本（草稿 3.5b 只写了三晚） | 不阻塞。第四晚起无结果文本 |

**已核实不缺的**：事件选项小字已按 GDD 11.6 重做完毕（全部事件只剩 7 条小字，无一是叙述句）；巡视、采集、果园、夜账前三晚的结果文本均已入库。

**另有一件不缺文本但缺代码的**：草稿 §06 的五个随机事件（走丢的牛、上门卖农具的人、铸都来的行商、淘井、第三幕的一天）正文齐全，但**随机事件池根本没有实现**，代码里没有任何抽取逻辑。这项不在阶段四至六的任何一张票里，是漏排的。
