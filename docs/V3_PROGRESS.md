...
**一 · 结局 4B 在真实对局里不可达。这是阶段五唯一的硬堵。**

GDD 5.5 给格雷格列了三条行动性信任途径（修缮马厩 +1、帮忙照顾马匹 +1、展现对土地的尊重 +1），代码里只实现了第一条。交谈性信任上限 +2，因此他的实际上限是 3，而第三条碎片 `clue_pos_horse_condition` 要求 4。

依赖链：格雷格第三条拿不到 → Day 27 蒂埃里那个选项的 `requiresFlag` 不满足 → `clue_pos_locate` 拿不到 → 位置线停在 2 条 → **4A 可达，4B 不可达**。

同一个问题在埃莱娜身上更彻底：GDD 5.5 给了她两条途径（保持尊重但不过分关注 +1、保护她不受牵连 +1），**一条都没有实现**，全庄园只有她没有任何行动性信任来源。她的上限是 +2，两条碎片分别要 3 和 4，整条线是死内容。

| NPC | 行动性信任现状 | 实际上限 | 后果 |
|---|---|---|---|
| 格雷格 | 修缮马厩屋顶 +1（缺两条） | 3 | 第三条碎片与整条位置线断掉，4B 不可达 |
| 埃莱娜 | 无 | 2 | 两条碎片全部不可达 |
| 玛莎 | Day 10 请愿 +1、Day 21 营地 +1 | 4 | 可达，但第二条碎片实际要求参加狩猎季 |
| 洛伦茨 | 首次守夜 +1、Day 21 那棵树 +1（本阶段补上） | 4 | 可达，同样绑定狩猎季 |

**这不是阶段五能自己解决的**：草稿没有给"帮忙照顾马匹""保护她不受牵连"任何正文或触发场合，GDD 也只给了名目。需要作者定这四条途径各自挂在哪个行动或事件上。碎片交付机制本身已经写完并测过，补上信任来源即可打通，不需要改 `ClueSystem`。
