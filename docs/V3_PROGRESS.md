...
- **维特第二档的重述**（草稿 4.11）。第二档的机制是"把玩家说的话按事情发生的顺序重排"，这需要 14 条碎片各有一句压缩重述，草稿里没有。当前第二档播放前后框架（"顺序好像不对"到"缺的那几块应该都在您认识的人手里"），中间的重排段落缺失。
- **Day 30 的三十天回顾**（草稿 4.13）。草稿标为"系统按玩家的实际行为组装"，列举的五项里"修了几户人家的屋顶"能从请愿 flag 推出，"在马厩待过多少个晚上"在现有行动里没有对应物（和格雷格交谈是下午行动）。当前播放前后两段，中间清单留空。要补的话需要先定这五项各自对应哪个计数。
