...
### Changed
- **理性封建**：去掉"独特之处："这类百科式起头；补上源文档里的"每位领主对上有义务、对下有担保"（与亨克档案的制度视角呼应），落回"你与冯·阿尔德的那纸担保书就是这套制度最日常的一角"。
- **圣火铸造教**：补全"锻造·淬炼·奉献·永恒"四概念框架；"待塑的生铁"改用源文档用词"原矿"（en `raw iron`→`raw ore`）。保留成立的对照（匠师不是牧师、有炉膛没祭坛——读者默认会当基督教，是源文档明确要纠的）。
- **瓦莱维斯普**：补上 Valewisp 招牌的"社交围着猎季与马匹转"（与马夫/两位贵族条目呼应）；源文档明确写出的对照"不是傲慢，是自觉活得最贴近土地的秩序"保留（上一轮曾误当稻草人删过，已还原）。
- **README.md**：刷新公开自述——补上**中英双语 + 游戏内切换**、**见闻（跨局 Compendium）**、调查线、离线单文件构建（`build:standalone`）、以及刷新状态表。保持无剧透（README 也随盲测包发出）。

### 备注
- 修订中一度因引号被工具混成 ASCII/全角不一致（结构引号与内层强调引号互换）导致 `zh/codex.json` JSON 失效，改用 Node 脚本以 `"`（结构引号）/ `“”`（内层"") 转义重写，正文不出现任何字面引号字符，规避歧义。
- 见闻只动内容层 `data/{zh,en}/codex.json`，`codex.ts` 结构不变；`CodexSystem` / `Localization` 结构与配对断言全绿。

## [2026-09-20] — 见闻人物精修（humanizer + 中文标点规范）· 归火神学进 WORLDBOOK

作者反馈:标点与文风要过一遍规范。三件事，全库 **576 绿**，tsc 干净，浏览器实测见闻新文案 + 中文引号渲染通过。

### Changed
- **六位人物再修**：按 `humanizer-zh`（中文 9 patterns）与 humanizer（英文 24 patterns）精修，并**更贴作者 `Project VALE` 原文**——去掉自造比喻（如马夫的"一根筋"）、暴力确定性词（亨克的"算…"改"看…"、"小算盘"改"小买卖"），保留原文里成立的对照与细节（马不会撒谎、每一年的火不是同一簇火、她知道得最多也最难开口）。中英成对。
- **中文标点规范**：全库 43 个 zh 数据文件本就用全角中文标点与中文引号 “”，唯独 codex 里我上一版残留了角括号 「」（日式/繁体约定）与聊天里混过的半角逗号。本次把 codex 全部 「」→ “”、确认 zh 正文无半角标点混入。**已记为长期标准**（见记忆 `text-punctuation-standard`）：zh 用全角中文标点 + “”，en 用弯引号 ’，每次文本更新都跑 humanizer。

### Added（docs，经作者批准）
- **`docs/WORLDBOOK.md` 新增两节**：
  - **归火（Death and the Return to the Flame）**：人生=锻造、死亡=身体交还圣火（最后一道工序，非销毁而是接纳）；火葬"圣火接纳" vs 火刑"圣火审判"同火异义；火刑留在法里却几乎不用——不是太残忍，是太神圣。这是**结局 4A + 洛伦茨仪式**的底层。
  - **身份证件**：全国统一护照 + 各公国在身份证上做文章（Valewisp 的植物纹样）；玩家持的是挂在领主担保上的临时工作许可，"成为河谷的人"更近于一纸读全了条款的契约。

### 备注
- WORLDBOOK 改动经作者明确批准（"归火神学以及一系列世界知识可以进 worldbook"）；docs/ 其余仍按 CLAUDE.md 不擅动。
- 王室层 `神君侧影` 仍只作私有 canon 参考，不进游戏或 WORLDBOOK。

## [2026-09-20] — 见闻六位人物正文（据作者档案）+ 两处设定订正

作者发来角色档案（`Project VALE` / `project_vale_characters_f1`，私稿，`design-drafts/` 已 gitignore）。按其中 Timothy 段确立的**三段模板**把见闻六位人物从占位改成正稿——三段正好落进已有的三层：**照面**（速写：身份·性格·外观·出身·价值）/ **底细**（与本地的关系）/ **原型**（专业底色 + 软化细节 +「这样的角色在玛里尼/瓦莱维斯普意味着什么」的制度视角）。只动内容层 `data/{zh,en}/codex.json`，结构不变。全库 **576 绿**，tsc 干净，浏览器实测三层解锁与渲染、两处订正均通过。

### Fixed（对照游戏文本核出的占位错误）
- **埃莱娜是「女仆」不是「文书」**：`day12` 明写“女仆负责办公室…收条是女仆归档的”。改 subtitle/silhouette 与正文——她扫房、顺手归档，所以“看得见东西”，而非账房文书。
- **亨克是「磨岭庄园」男爵，不是「棘墙·玛格丽特的丈夫」**：`day18` 猎季邀请“从磨岭庄园赶来…亨克男爵亲笔”。占位把他误写成玛格丽特的丈夫、坐落棘墙；订正为磨岭男爵（与 hidden 的「磨岭一角」条目一致）。

### Changed
- 六位人物（gregor / marta / elena / lorenz / marguerite / henk）三层正文全部重写，中英成对。语气沿用见闻既有的冷调、非数字、非剧透；较深的调查线仍只留在每局重置的「卷宗」，见闻不剧透。
- `von Alde` / `Valehold` / `Thornwall` / `Millridge` 等既定英文名与游戏内统一（弃用档案里的 `Ludwig` 名，改用游戏用的「冯·阿尔德男爵」）。

### 备注
- 世界知识条目（尤其**归火神学**、Valewisp 公国身份）档案里素材充足，是下一步；若要落进 `docs/WORLDBOOK.md` 需作者另行批准（docs/ 按 CLAUDE.md 规矩不擅动）。
- 王室层材料（`神君侧影`）远高于本作海拔且含私密内容，只作私有 canon 一致性参考，不进游戏文本。

## [2026-09-20] — 第四轮盲测修复：跨语言读档语言锁 + 结局 4A 时间线 + 见闻占位

第四轮外部盲测（读文本 + 部分交互）三条，均先核实属实再修。全库 **576 绿**，tsc 干净，浏览器实测跨语言读档链路通过。

### Fixed
- **切语言后读旧档语言混用**：存档存的是"已按某语言渲染好的"场景文与行动标签（当初为 PlaytestFeedback 4.h "读档不重掷"故意如此），切语言重载后 UI 外壳换了、烤死文本没换 → 混用。方案：把 `locale` 戳进存档**元数据**（不进 `GameState`，不动 `SAVE_VERSION`；旧档缺该字段则按当前语言读、维持旧行为）。读档时若存档语言≠当前语言，先 `setLocale(存档语言)`（会 reload），用一次性 `valley-season:resume` 标记在重载后自动重开该槽——**语言开关只在标题页，故不破坏任何局内功能**；读档后整季永远同语言。`SaveSystem`：`SaveSummary.locale`、`packSave` 戳记、`readSummary` 回读 + 旧档回退、`setPendingResume` / `takePendingResume`。`App`：`openSlot` 统一继续/读档并做语言对齐，boot 时消费续档标记。
- **结局 4A（一具没有归火的尸体）时间线自相矛盾**：`rites_open` 写"圣火节之后第三天"、`festival_mirror` 写"圣火节在归火之后第三天"，两句互为反向。以 `festival_mirror` 正文"该办的事一件一件办，办完了接着过节"为准（归火在前），改 `rites_open`："之后"→"之前"（en `after`→`before`）。zh/en 各一词。
- **「磨岭一角」见闻解锁占位剧透**：`millridge` 条目（`reveal:'hidden'`，挂 `clue_mot_handwriting`）正文是带"剧透线 / 待按真相线补写"字样的占位——虽躲在调查线索后，但半成品且能被 grep。换成世界观内、无剧透的中性 stub（"河谷一带的一处庄园。你只听过这个名字…"），待作者文档替换。

### Tests
- `SaveSystem.test` +6：locale 戳记（当前语言 / 显式 / 旧档回退）；续档标记一次性交回、无标记、无 storage 不抛。

### 备注
- 混用另一条解法（改存 id、渲染时取字）会动到刚稳定的 advance 管线且与"不重掷"冲突，未采。
- 见闻 `millridge` 与结局 4A 均为**叙事文本改动**：4A 方向经作者确认；见闻占位仍待作者正式档案替换。

## [2026-09-20] — 声画数据接缝（SFX/VFX：先接缝、无资源）

第三轮反馈 SFX/VFX。作者与我共识：这是**安静**的游戏，声画只做 diegetic、克制，且要做就**数据驱动**。本条只铺**接缝**、不产资源：一张"世界状态 → cue id"的映射 + 一个纯函数解析器。播放器尚未接（无音频对象、无副作用、UI 未接），日后点亮声/画只改数据与播放器、不碰任何游戏逻辑。全库 **570 绿**，tsc 干净。

### Added
- **`src/data/cues.ts`**：`CUES` 注册表（cue id + channel + 空 `src`，待有资源再填）；`AMBIENT_BY_WEATHER` / `VFX_BY_WEATHER`（按真实 `WeatherType`）；`AMBIENT_BY_SCENE`（起步映射 fields / forest / office，可扩）。
- **`src/systems/CueSystem.ts` · `resolveCues(state)`**：纯读，按 channel 返回 cue id（去空、去重、只留已注册的）。`mappedCueIds()` 供测试防打错 id。

### Tests
- `CueSystem.test`（4 条）：天气床 + 地点床叠加；晴天 + 未映射场景取空；霜冻的床与层；映射 id 全部在 `CUES` 注册。

### 备注
- 无资源、无播放器、UI 未接——这是"接缝"，不是功能。合味的用法：像状态栏的 frost 强调色那样克制、diegetic。声音的包袱（浏览器自动播放限制、静音开关、内联进单文件会撑大体积）留到真做时再处理。
...
### Changed
- **"无产出"改为说清行动到底给什么**（`actions.json` 中英，`common.noYield` 移除）。它只出现在两张卡上，而两者都不是真的无产出：**巡视农田** 置 `surveyedFields`，让佃户请愿走"已了解情况"那条路（`EventSystem` ~161）；**夜间审查账目** 第三夜出 `clue_mot_handwriting`。巡视农田现按巡视林地的样式给出用途句（不剧透事件）；夜查账目改为氛围句（不剧透线索），保留 `疲劳 +1`。
- **关系类微文案去数字化**（`actions.json` 中英）。奖励用人话说：修马厩 `格雷格信任 +1`→`格雷格记着这份好`、伴手礼 `X信任 +1`→`X领这份情`、果园 `佃户整体信任 +1`→`佃户领这份情`；毁约伐木卡不再显真实惩罚数值（`格雷格信任 {trust}`→`格雷格会记在心里`，−2 仍照常施加于 `relationships.gregor`）。交谈计数改为社交副产品的自然说法：`计一次与X的交谈`→`顺道和X搭上话 / 照面 / 说会儿话` 等；找格雷格 `不计信任`→`只是打个招呼`（仍以"打招呼 vs 搭把手"在文中区分哪次算数）。佃户会议门槛也去数字：`佃户整体信任需 ≥ N`→`佃户整体信任未达`（是"未达"门槛而非奖励，但同样不必示数；`EstateTaskSystem` 相应去掉 `fill` 的 `{n}`，`TENANT_MEETING_MIN_TRUST` 仍用于真正的判定）。**保留**功能性数字：资源 chip、收割 / 伐木 tier、声望 `+1`（声望是刻意可见的公开分）。
- **每日选项由 tab 改为分组堆叠**（`ChoicePanel`）。原先 劳作 / 往来 / 休整 三个 tab、一次只显一类——玩家点来点去没欲望、反而忘了别类还有事。改为三类按序竖排、各带小标题一屏扫完，空类别自动隐去；沿用既有 `getChoiceCategory` 归类与 `lg:hidden` 委派（事务 / 集市在桌面仍走左栏），移除 tab 状态与 `ui.json` 的 `tabEmpty`（中英）。契合单阅读列方向（[[ui-layout-decision-pending]]）。

### Tests
- `EstateTaskSystem.test`：修马厩摘要断言随文案改为 `…格雷格记着这份好`；describe 标题相应改为"成本按数字、关系收益按词而非数字（GDD 11.6 / D3）"。
- `ClueSystem.test` / `EventSystem.test`：搭把手、找格雷格两处描述断言随文案更新。

### 备注
- **与 GDD 11.6 的张力**：11.6 定"微文案机械、不叙事"，本轮把关系类收益改成词句（承作者本轮决定、向 D3 收敛）。GDD 未改（需作者批准）；11.6 现描述代码略失准，建议作者择机把"关系收益不显数字"并入 11.6 / D3。
- **未纳入本轮**（作者定：放着）：`和玛莎聊聊` 的 `玛莎最近话比以前多了——这是信任的表现`（点名"信任"但无数字、偏叙事，故保留）。
- **①开场压缩**：暂缓。注：标题页"新的一季"后已有 **跳过开场** 入口（担保书页即可跳到 Day 1），压缩正文与否是纯叙事取舍。

## [2026-09-14] — 响应式：窄屏把三栏叠成一列

此前是桌面优先的固定三栏，`lg`（1024px）以下整条左栏（庄园事务＋记录）直接 `hidden`，且三栏不换行——在 375px 手机上右侧 256px 的状态栏挤得正文只剩 ~80px，几乎没法读。改为窄屏叠成单列滚动，桌面三栏不变。浏览器实测 375 / 750 / 1280 三档，全库 **553 绿**，tsc 干净。

### Changed
- **`lg` 以下三栏叠成一列**（`App.tsx` 外层 `flex-col lg:flex-row` + `overflow-y-auto lg:overflow-visible`）。窄屏顺序：正文＋选择（满宽）→ 状态栏（满宽）→ 记录。桌面（≥1024px）仍是 事务/记录 · 正文 · 状态 三栏，`max-w-[78rem]` 居中封顶不变。
- **正文列不再被压成一条缝**：中栏 `flex-1 min-w-0` → `min-w-0 lg:flex-1`（窄屏取自然宽＝满宽）；状态列 `w-64` → `w-full lg:w-64`。
- **面板按屏内滚改为随页滚**：`ScenePanel` / `StatusPanel` 的 `h-full` 与内部 `overflow-y-auto` 都加 `lg:` 前缀——桌面各栏内部独立滚动照旧，窄屏改为整页一条滚动、面板按内容自然伸展。
- **记录在窄屏有了落脚点**：左栏在窄屏被收起，`记录` 于是在滚动末尾补一份 `lg:hidden` 的 `LogDrawer`；庄园事务本就在窄屏折回 `劳作` tab（`ChoicePanel` 的 `lg:hidden` 委派），故不丢。

## [2026-09-14] — 第二轮盲测④：维特 tier3 加门槛 + 结局 4A 开头改写

第二轮反馈里"正文声称玩家掌握了未获得的信息"的两处。作者定：tier3 收紧门槛而非拆碎（它就是最接近"彻底还原"的那档回答）；4A 开头不再让玩家交出他没拿到的马匹证据与布包。全库 **553 绿**，tsc 干净。

### Changed
- **维特 tier3 需持有 `clue_mot_martha_summer` 与 `clue_mot_elena_burned`**（`EventSystem.processWynter`）。tier3 是一整段连贯独白、点名了"夏天不再回应"与"走前那夜烧了东西"两件事的**内容**；此前只要线索总数 ≥ 6 就播，凑够 6 条却没拿到焚纸的玩家会听维特讲他们没获得的焚纸。现要求这两条到手才给 tier3，否则无论几条都退回 tier2（按形状点数、逐条守卫，本就诚实）。tier4（位置线）门控不变。
- **结局 4A 开头改写**（`endings.json` 中英）。4A 特意允许在位置线不完整（乃至为零）时进入，但开头写死了玩家"交给蒂埃里一个装草茎的布包、一张记着马蹄磨损方向与腿上泥高度的纸"——位置=0 的玩家从没拿到这些。改为：玩家交出的是他**弄明白的那个人**（为什么走、走了多久、七个星期），并说自己判断不出他在哪；由蒂埃里以本区护林人的身份去走他想得到的那几处。既不再凭空给玩家安上证物，也与 4B（位置线齐、蒂埃里凭草一口定位）形成正确对照。门槛（GDD 9.5）不动。

### Tests
- `Wynter.test`：`MOTIVE` 夹具补 `clue_mot_elena_burned`；新增"六条但缺焚纸 → 退回 tier2、不设 `wynterRestated`"。
- `EventSystem.test`：Day 22 "carrying" 态补 `clue_mot_elena_burned`，仍触发 tier3。
- 结局 4A 既有断言只查后半段（十二公里 / 埃莱娜 / 路德维希 / 尾声），开头改写不影响。

### 备注
- tier3 里"他去问过人，教会的、贵族的、公家的"一句仍未单独守卫贵族线（`clue_nob_marguerite`）——按作者本轮定的两条门槛未纳入；此为软性归类陈述，若日后要更严可再收。

## [2026-09-14] — 第二轮盲测续：移除账目孤儿行动 + 卷宗可展开原话

接前一条。作者定：③移除孤儿行动、⑤手记兜底一并做，④维特/4A 正文留待更多讨论。全库 **552 绿**，tsc 干净，浏览器实测 ⑤ 通过。

### Removed
- **`深查账目付款记录`（`deep_investigate_ledger`）**——v2 调查链遗留。它写 `deepInvestigatedLedger`（无人读）、消失判 `foundLedgerSource`（无人写），到 Day 12 前可反复点、耗时耗疲劳而无产出。v3 里"自己查账目"的线索是**夜查账目 → `clue_mot_handwriting`**（`review_accounts` 第三夜，照旧生效）；付款去向/磨岭一角按 GDD 9.4 是"反向线索、不可主动获得，在维特重述里自动闭合"，故此行动在 v3 无落点。连带删 `actions.json` 的 `deepLedger` 词条（中英）与两条只测可见性的旧断言。

### Added
- **卷宗可展开原话**（`JournalSystem` + `Journal.tsx`）。卷宗此前只存简短目录行；晚间正文一旦跨日被清（已由前一条修复），那行目录曾是玛莎 322 字唯一的留痕。现每条线索在目录行之外携带其**原文段落**：住户碎片取 `fragments[key].text`、公务员碎片取对应选项的 `resultText`、玛格丽特取马车整段。卷宗里点"展开原话"即就地展开原文，再点收起。夜查账目的笔迹线索是随机生成的场景结果、无单一定本，故不提供展开（只留目录行）。数据管线沿用既有的"按本局记录匹配候选行"逻辑，`fullCandidates` 与 `candidates` 同序对齐。

### Tests
- `JournalSystem.test`：住户碎片展开到其原话（目录 ≠ 原文，且原文 = `fragments.marta_lastwords.text`）；夜查账目无展开；双源线索（蒂埃里巡林范围）展开取用与本局目录同源的那段。

## [2026-09-14] — 第二轮盲测：三处纯 bug 修复（继续节拍 / 疲劳照播 / 分资源结算）

外部盲测者报了六处"系统进度与玩家读到的内容不一致"，逐条核到代码，六条全真。本次先修其中三处纯 bug；另有三处（深查账目的孤儿行动、维特 tier3 与结局 4A 的失守正文、手记兜底）涉及内容/文字决策，待作者定夺后再动。全库 **551 绿**，tsc 干净，浏览器实测 #1 通过。

### Fixed
- **晚间行动的结果文字被跨日清空**（`App.tsx`：新增 `commitAdvance` + `COMMIT_ADVANCE` + `GameState.pendingAdvance`）。晚间是当天最后一个时段，任何晚间行动会在同一次 dispatch 里跨到次日、被 `advancePhase` 的跨日重置清掉 `lastResult`，React 只渲染最终态——玩家永远读不到玛莎的临别话、埃莱娜的焚纸、夜查账目的收束等晚间正文（线索 flag 已写入，正文却没了）。现对"在当日最后一个时段、且有结果可读"的行动延后跨日：正文停在结果上，加一个"继续 →"节拍，玩家点了才翻天。上午/下午行动不变。
- **疲劳跳过上午也吞掉清晨固定事件**（`App.tsx` 强制休息分支 + `EventSystem.hasMorningFixedEvent`）。次日疲劳达上限会把 `phase` 直接推到下午，而 `getFixedEvent` 按时段匹配，`dawn`/晨间固定事件（Day 18 邀请信 → 开狩猎季；Day 23 文秘室来信 → 经纪人渠道）在下午匹配不到，永久丢失。改为：当天有晨间固定事件时不跳过上午，让事件照播（照旧重置疲劳）；无晨间事件的日子仍照常跳过上午。硬编码的 `>= 5` 一并改回 `FATIGUE_EXHAUSTED_THRESHOLD`。
- **结算文本按单一资源判定**（`EventSystem.processDay30` + `day30_evening` 正文）。原本只看粮食 ≥ 75 就走 `settled`，正文写死"金卢有结余、木材留足取暖"，与实际金卢/木材无关（75/0/0 会照写有结余）。现按粮/金卢/木材各自的实际状态分句组装：粮食分"够春播 / 种子要紧着分"，金卢分"有结余 / 一分不剩但不欠"，木材分"留足 / 见了底"，收束段照旧。

### Tests
- `EventSystem.test`：`hasMorningFixedEvent`（Day 18/23 晨间事件为真；Day 22 黄昏、平常日为假）；Day 30 结算分资源（富足态含"春播/有结余/留取暖"；75/0/0 态含"种子要紧着分/一分不剩/见了底"且不误报有结余）。

### 待定（本轮未动，等作者决定）
- **`深查账目付款记录`（`deep_investigate_ledger`）是 v2 调查链遗留**：写 `deepInvestigatedLedger`（无人读）、消失判 `foundLedgerSource`（无人写），到 Day 12 前可反复点、无产出。v3 里"自己查账目"的线索其实是**夜查账目 → `clue_mot_handwriting`**（`review_accounts` 第三夜，已生效）；付款去向/磨岭一角按 GDD 9.4 是"反向线索、不可主动获得"。建议移除该孤儿行动（有两条只测可见性的旧测试），或若要做成真线索则需改 GDD 9。
- 维特 tier3（`countClues ≥ 6` 整段播，含未获得的焚纸细节）与结局 4A（位置线为零仍描写交出马匹证据）——需按 tier2 的逐条守卫方式重写正文，作者主导。
...
### 补齐：贵族信任的第二次机会（边界纠纷）
GDD 5.5 列贵族信任"三次机会"含"Day 15 之后·边界纠纷中尊重历史、不自作聪明 +1"，此前只有晚宴与 Day 18 猎场两处落地，贵族信任实际最高只到 2、"容错一次"不成立。不必新建场景：Day 15 树桩前对蒂埃里的**克制回应 `stumps_dont_know`（"这算不算越界我判断不了"）就是"不自作聪明"的得体之举**，给它加 `nobleTrust +1`（中英成对）。相对的 `stumps_accuse`（无凭据断定越界）是自作聪明的越界，不给。三次机会至此齐备。
...
- **`src/components/common/NameEntry.tsx`**——先输名字的独立开场屏（D6）。
- 选项**颜色 chip** + hover 描边 + active 按下反馈（`ChoicePanel`，D11/P7/P9）。
