...
> - 🔶 **阶段二（UI）进行中**：✅ 2.1 选项 chip+hover+按下、2.2 关系模糊化+佃户行、2.3 左栏可点+集市置顶、2.4 开场先输名+跳过、2.5 过劳可见、2.6 对话头像框——均已浏览器验证。⬜ 2.7 流言/狩猎季、2.8 微文案、三个打磨点未做（见下「恢复点」）。
> - ⬜ **阶段三（文本精简 D2）** 未开始。
> - 全库 528 测试绿，tsc 干净。
>
...
> ---
> ## 🔖 恢复点（compact 之后从这里继续）
> 阶段二还剩下面这些，**方向都已和作者确认**，直接实现即可（改完照例 tsc + `npm test` + 浏览器验证 + 更新 CHANGELOG，最后 commit）：
> 1. **D12 流言衔接（P21）**：`src/systems/SceneSystem.ts` 里 `market_rumours`/`readRumours`（约 153、195、240 行）与 `src/data/{en,zh}/scenes/rumors.json`。给流言前加"听到有人说……"式引子，别让它突兀地接在集市正文后。中英成对。
> 2. **D13 狩猎季 UI（P23）**：Day18–22（`flags.huntingSeasonStarted && day∈[HUNT_FIRST_DAY,HUNT_LAST_DAY]`）——换氛围色 + 顶部季节标签。改 `ScenePanel`（和/或 App 布局），新增 `ui.json` 标签文案（中英）。
> 3. **打磨-1**：`actions.json`（中英）forage/orchard 的 description 去掉"{n} 金卢"，只留 chip（`ChoicePreview` 已给 `🪙 +N`）。注意别动 log 文案。
> 4. **打磨-2**：`EstateTaskList` 左栏任务花费**悬停**变锈红（用 group-hover）。
> 5. **打磨-3**：`ChoicePanel` 里收割/采伐（有档位词、`resultKind` 为 harvest/fell_timber）的 description 给淡金（如 `text-gold-dim`），暗示收益。
> 6. **P27（延后）**：不做，等作者下次 playtest 想起具体指哪句。
> 7. 之后进 **阶段三（D2 文本精简）**。
> ---
